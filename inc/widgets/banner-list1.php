 ...
 */
class tcdw_banner_list1_widget extends WP_Widget {
  ...
  function __construct() {
    parent::__construct(
      'tcdw_banner_list1_widget',// ID
      __( 'Banner list1 (tcd ver)', 'tcd-w' ),
      array(
        'classname' => 'tcdw_banner_list1_widget',
    ...
    for ( $i = 1; $i <= $this->banner_count; $i++ ) {
      ...
      $banner_image = wp_get_attachment_image_src( $instance['banner_image' . $i], 'size3' );

      $output .= '<li class="p-banners-list__item p-banner">';
      ...
      $output .= '<p>' . nl2br( esc_html( $banner_title ) ) . '</p>';
...
?>
  ...
  <div class="a-widget-box">
    ...
    <div class="a-widget-box__content">
      ...
      <p><textarea id="<?php echo $this->get_field_id( 'banner_title' . $i ); ?>" name="<?php echo $this->get_field_name( 'banner_title' . $i ); ?>" class="widefat"><?php echo esc_textarea( $instance['banner_title' . $i] ); ?></textarea></p>
      <h5 class="a-widget-box__sub-headline">
...
<?php
  ...
  function update( $new_instance, $old_instance ) {
    ...
    for($i = 1; $i <= $this->banner_count; $i++) {
      ...
      $new_instance['banner_display_gradation_overlay' . $i] = $new_instance['banner_display_gradation_overlay' . $i] ? '1' : null;
...
// register tcdw_banner_list1_widget widget
function register_tcdw_banner_list1_widget() {
	register_widget( 'tcdw_banner_list1_widget' );
}
add_action( 'widgets_init', 'register_tcdw_banner_list1_widget' );
