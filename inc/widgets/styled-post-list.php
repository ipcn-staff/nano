...
class Styled_Post_List_Widget extends WP_Widget {
  ...
  <div class="p-tab-list p-widget">
    ...
    <ul class="p-tab-list__tab">
      <?php for ( $i = 1; $i <= 2; $i++ ) : ?>
        <?php if ( $instance['display' . $i] ) : ?>
          <li class="p-tab-list__tab-item<?php if ( 1 === $i ) { echo ' is-active'; } ?>">
            <a href="#panel<?php echo esc_attr( $widget_id ); ?>-<?php echo $i; ?>">
              <?php echo esc_html( $instance['title' . $i] ); ?>
            </a>
          </li>
        <?php endif; ?>
      <?php endfor; ?>
    ...
    ?>
    <div id="panel<?php echo esc_attr( $widget_id ); ?>-<?php echo $i; ?>" class="p-tab-list__panel<?php if ( 1 === $i ) { echo ' is-active'; } ?>">
      ...
      <article class="p-tab-list__panel-item p-article02 u-clearfix">
        <a href="<?php the_permalink(); ?>" class="p-hover-effect--<?php echo esc_attr( $dp_options['hover_type'] ); ?>">
          ...
          <div class="p-article02__content">
            <h3 class="p-article02__title"><?php echo is_mobile() ? wp_trim_words( get_the_title(), 20, '...' ) : wp_trim_words( get_the_title(), 25, '...' ); ?></h3>
  ...
  <?php
    ...
    for ( $i = 1; $i <= 2; $i++ ) {
      ...
      $new_instance['display' . $i] = $new_instance['display' . $i] ? 1 : null;
      if ( ! isset( $new_instance['display_date' . $i] ) ) $new_instance['display_date' . $i] = null;
      $new_instance['display_date' . $i] = $new_instance['display_date' . $i] ? 1 : null;
